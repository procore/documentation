...
The Procore API implementation of the Client Credentials flow relies on the use of a [_Developer Managed Service Account (DMSA)_]({{ site.url }}{{ site.baseurl }}{% link plan_your_app/developer_managed_service_accounts.md %}).
...
The client application requests an access token using its own credentials through the DMSA which, based on the tool permissions you specify, determines the tools and projects the application can access.
...
The JSON response block from the call includes an `access_token` string and additional attributes as shown here:
...
For additional information on MPR, see [Working with Multiple Procore Regions]({{ site.url }}{{ site.baseurl }}{% link tutorials/tutorial_mpz.md %}).
