...
These are used by the storage service to verify data integrity.
...
```
{
  "data": {
    ...
    "segments": [
      {
        "url": "https://s3.amazonaws.com/pro-core.com/companies/123/01JEXAMPLE00000000000000001?...",
...
Upload the binary file content to the presigned URL using the headers from the previous step.

**Request**

- Method: `PUT`
- URL: The `url` value from the segment
- Headers: The `headers` from the segment
- Body: Raw binary file content
- Auth: None (the URL is pre-authenticated)

```
curl -X PUT '{segment_url}' \
...
```
{
  "data": {
    ...
    "segments": [
      {
        "url": "https://s3.amazonaws.com/pro-core.com/companies/123/01JEXAMPLE00000000000000002?partNumber=1&...",
        "url_expires_at": 1773900000,
        "headers": {
      ...
      {
        "url": "https://s3.amazonaws.com/pro-core.com/companies/123/01JEXAMPLE00000000000000002?partNumber=2&...",
        "url_expires_at": 1773900000,
        "headers": {
          "Content-Type": "video/mp4",
...
Upload each part to its corresponding presigned URL using the headers from the response.
Parts can be uploaded in parallel for better performance.

**Part 1:**

```
curl -X PUT '{segment_1_url}' \
  --header 'Content-Type: video/mp4' \
  --header 'Content-Length: 6000000' \
  --header 'x-amz-content-sha256: b2a6304fdd19da95f8750573f5fd33e0ad71c3a41b1b6daaf4621fd9af913952' \
  --header 'content-md5: ZfoBY1ehgnLOCG/0aUumGg==' \
  --data-binary '@part_aa'
```
...
```
curl -X PUT '{segment_2_url}' \
  --header 'Content-Type: video/mp4' \
  --header 'Content-Length: 2829449' \
  --header 'x-amz-content-sha256: 9b9ffee440b0936c280d49f42ceb554eb3bf404b0604e95da1783bc3d64d58e7' \
  --header 'content-md5: CHWHKcOPMIG50LvmsN5B+g==' \
  --data-binary '@part_ab'
```
...
- **Presigned URLs expire.** The `url_expires_at` field indicates when the presigned URL becomes invalid. If a URL expires before you complete the PUT, use the GET URL endpoints to obtain a fresh presigned URL.
- **Use the exact headers from the response.** The presigned URL is signed with specific headers (Content-Type, Content-Length, checksums). Sending different headers will result in a `SignatureDoesNotMatch` error from the storage service.
- **Do not include your access token in PUT requests.** The presigned URL is pre-authenticated for the storage service. Including an Authorization header in the PUT will cause an error.
