...
- [Register for a Developer Account]({% link getting_started/new_account.md %}) on the Procore Developer Portal.
- [Create an Application]({% link building_applications/building_apps_intro.md %}) using your Developer Account.

In addition, you will need to familiarize yourself with the [OAuth 2.0 authentication protocol]({% link oauth/oauth_introduction.md %}) as access to the Procore API is secured by the authorization and authentication requirements of OAuth 2.0.
...
Before you can make a call to the Procore API using Postman, you must [configure OAuth 2.0 authorization]({% link tools/postman.md %}#postman-token-gen) using Postman's token management tool.
...
- The Authorization Type is set to 'Inherit auth from parent' because we have configured OAuth 2.0 in Postman at the collection level as described in [Generating OAuth 2.0 Tokens]({% link tools/postman.md %}#postman-token-gen).

In addition to the settings described above, set the `Procore-Company-ID` header value as needed to work with [Multiple Procore Zones (MPZ)]({% link tutorials/tutorial_mpz.md %}).
