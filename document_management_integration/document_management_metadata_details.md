...
| **placeholder_created_by** | reference | User who created the placeholder. Defaults to the authenticated user at creation, but can be overridden via API. |
...
| **uploaded_by** | reference | User who uploaded the document. Defaults to the authenticated user at creation, but can be overridden via API. |
...
Placeholder fields enable creation of document uploads for files that are expected to be delivered at a future date. These fields act as container for anticipated document information, such as name, type, due date, and the responsible party. Once the actual file is ready, it can be uploaded and linked to the corresponding placeholder. Editable placeholder fields (`placeholder_assignee`, `placeholder_created_by`, and `placeholder_due_date`) are listed in the [Standard Metadata Fields](#standard-metadata-fields) section.
