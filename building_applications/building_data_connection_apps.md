...
Find them on the **Credentials** tab of your app. Credentials follow this component, so there is nothing to collect on that tab until you add it. Sandbox credentials are generated once a version carries a Data Connector component; production credentials are populated once you promote such a version to production. See [OAuth Credentials Management]({{ site.url }}{{ site.baseurl }}{% link oauth/oauth_keys.md %}) to manage them and your redirect URIs.
